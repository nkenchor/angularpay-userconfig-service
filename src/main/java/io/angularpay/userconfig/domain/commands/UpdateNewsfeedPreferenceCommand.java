...
import io.angularpay.userconfig.models.UpdateNewsfeedPreferenceCommandRequest;
...
@Service
public class UpdateNewsfeedPreferenceCommand extends AbstractCommand<UpdateNewsfeedPreferenceCommandRequest, GenericReferenceResponse>
    ...
    public UpdateNewsfeedPreferenceCommand(
            ObjectMapper mapper,
            MongoAdapter mongoAdapter,
            DefaultConstraintValidator validator,
            CommandHelper commandHelper) {
        super("UpdateNewsfeedPreferenceCommand", mapper);
    ...
    @Override
    protected String getResourceOwner(UpdateNewsfeedPreferenceCommandRequest request) {
        return this.commandHelper.getRequestOwner(request.getUserReference());
    }

    @Override
    protected TreeReferenceResponse handle(UpdateNewsfeedPreferenceCommandRequest request) {
        UserConfiguration found = getRequestByReferenceOrThrow(this.mongoAdapter, request.getUserReference());
        validRequestStatusOrThrow(found);
        found.getNewsfeedPreferences().forEach(x -> {
            if (request.getServiceReference().equalsIgnoreCase(x.getServiceReference())) {
                x.setShowInNewsfeed(request.getNewsfeedPreferenceApiModel().getShowInNewsfeed());
                x.setOnlyShowStatusTypes(request.getNewsfeedPreferenceApiModel().getOnlyShowStatusTypes());
            }
        });
    ...
    @Override
    protected List<ErrorObject> validate(UpdateNewsfeedPreferenceCommandRequest request) {
